        ...
        <div class="col liner">
            <div class="row">
                <div class="col logo d-flex justify-content-center">
                    <svg width="280" height="49" viewBox="0 0 228 39" fill="none"
            ...
            <div class="row">
                <div class="col text-one">
                    <router-link to="#" class="first-link">1books</router-link>
                    {{$t('login.about1book')}}
                </div>
            </div>
            ...
            <div class="row">
                <div class="col">
                    <div class="form-input">
                        <span class="icon">
                            <img src="../../images/tel-logo.png" alt="" class="fa fa-envelope" aria-hidden="true">
                        </span>
                        ...
                        <input
                            ...
                            class="form-control"
                            v-model="number"
                            @blur="filterPhoneNumber"
                            placeholder="+998(--) --- -- --"
                            maxlength="18"
                            @input="defaultVal"
                            pattern="^\+[0-9]{3}\([0-9]{2}\) [0-9]{3}-[0-9]{2}-[0-9]{2}"
                            :class="{'is-invalid': errorInputPhoneMessage}"
                        >
                    </div>
                </div>
            </div>

            <div class="errorInputMessage" v-if="errorInputPhoneMessage">{{$t('security.badFormatPhoneNumber')}}</div>

            <div class="row" v-if="passwordInput" style="margin-top: -20px;">
                <div class="col">
                    <div class="form-input">
                        <span class="icon">
                            <img src="../../images/lock.png" alt="" class="fa fa-envelope" aria-hidden="true">
                        </span>
                        <input
                            type="password"
                            name="email"
                    ...
                    <div class="errorInputMessage" v-if="errorPassword"> {{$t('security.passwordWrong')}} </div>
                </div>
            </div>

            <Button text="logIn" :onClick="auth" type="submit"/>

            <div class="row">
                <div class="col text-two">
                    {{$t('login.agreement.first')}}
                    <router-link to="#" class="second-link">{{$t('login.agreement.second')}}</router-link>
                    {{$t('login.agreement.third')}}
                    <router-link to="#" class="second-link">{{$t('login.agreement.fourth')}}</router-link>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col language">
                    ...
                    <router-link to="#">{{$t('login.selectLanguage.lanLogo')}}</router-link>
                    <br/> {{ $t('login.selectLanguage.for') }}
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        filterPhoneNumber() {
            ...
            this.phone = result[0] + result[1] + result[2] + result[3] + result[4]
        },
...
<style scoped>
.all {
    background: url("../../images/login-fourth-logo2.png");
